...
#define DEFAULT_DESIRED_MIN_CONN_INTERVAL     20//4//80//  连接间隔与数据发送量有关， 连接间隔越短， 单位时间内就能发送越多的数据

// Maximum connection interval (units of 1.25ms, 800=1000ms) if automatic parameter update request is enabled
#define DEFAULT_DESIRED_MAX_CONN_INTERVAL     80//10//800//  连接间隔与数据发送量有关， 连接间隔越短， 单位时间内就能发送越多的数据
...
// GAP GATT Attributes
static uint8 attDeviceName[GAP_DEVICE_NAME_LEN] = "BJC";
...
//  uint8 responseData[8] = {0x00};
    uint8 * responseData = osal_mem_alloc(3);
...
//            if(sys_config.Phone_User[0][0] == 0xFF)
//            {
//               sys_24c64.Phone_Num = 0;
//            }
//            else
//            {
//              sys_24c64.Phone_Num ++ ;
...
//        global.genPswdFlag = 0;							 // 状态位清零
   ...
   if( global.MY_KEY_CODE  )
   {
      ...
      {
        if(osal_memcmpn(&sys_24c64.General_Pswd[i][1], global.MvInPswd, 8))      //  普通密码
        {
        ...
        else if(osal_memcmpn(&sys_24c64.Once_Single[i][1], global.MvInPswd, 8))   // 一次性密码 单次
        {
          Option_Auto = TRUE;//Auto_Open;
          KeyLock_InMode();
          ...
          osal_memcpyz(&sys_24c64.Once_Single[i][1], 0xBB, 9);                             // 同时删除一次性密码
          osal_set_event(bleAutoLock_TaskID, DOOR_OPENDOOR_EVT);
...
//      osal_start_timerEx( bleDoorLock_TaskID, DOOR_ENCRYPT_EVT, 10 ); 
    ...
    osal_start_timerEx( bleAutoLock_TaskID, DOOR_ENCRYPT_EVT, 200 ); 
...
//      global.openDoorTurn = 0;
      ...
      if(sys_recode.Open_Count >=21)
      {
        sys_recode.Open_Count = 20;
      ...
      sys_recode.open_DoorRec[0] ++;                                                         //开关门记录标志
            ...
            case 5:              // 删除手机用户xx?
              {
                ...
                if( Button_Num == 0x30 )    // 下
                {
                  ...
                  for(uint8 i=global.delPhoneNum;i<10;i++)
                  {
                    global.delPhoneNum ++;
                    ...
                    {
                      OLED_DELKEYS_NUM(global.delPhoneNum);
                      break;
                    }
                  }
                }
...
//                OLED_DELPHONE_NUM(global.delPhoneNum);
            ...
            case 7:          
              {
                uint8 * completKeyCode = osal_mem_alloc(16);
                ...
                if( Button_Num == 0x0B )
                {
                  ...
                  else if(global.MvInPswd[2] == 0x31 && global.MvInPswd[3] == 0x31 && global.MvInPswd[4] == 0x30)
                  {
                    osal_stop_timerEx( bleAutoLock_TaskID, DOOR_PHONE_EVT);             //
                    osal_stop_timerEx( bleAutoLock_TaskID, DOOR_ENCRYPT_EVT);                 //
                    osal_stop_timerEx( bleAutoLock_TaskID, DOOR_PERIODIC_EVT);                 //
                    osal_stop_timerEx( bleAutoLock_TaskID, DOOR_POWERONOFF_EVT ); 
                    osal_stop_timerEx( bleAutoLock_TaskID, DOOR_ADMININ_EVT);             //
                    osal_stop_timerEx( bleAutoLock_TaskID, DOOR_ADMIN_PRO_EVT);             //
                    osal_stop_timerEx(bleAutoLock_TaskID, DOOR_OUTTIME_EVT);
                    osal_stop_timerEx( bleAutoLock_TaskID, DOOR_PSWD_OPEN_EVT);             //
                    ...
                    while( ((temp2+192)>temp1) || ((temp2>temp1)&&(temp2<(temp1+576))) || ((temp2+192)>0x1FF0) )
                    {
                      temp2 = RandomNumberGenerator();
                    }
        ...
        case 0x02:
          {
            ...
            uint8 * varUpdata2 = osal_mem_alloc(3);
            static uint8 falseReq[] = {0, 0x43, 0x57};
            uint8 * tempReq = osal_mem_alloc(20);
            uint8 * tempReq2 = osal_mem_alloc(20);
          ...
          {     
            ...
            case 0x42:                                                          //  管理员密码
              if(global.appUpdata_CRC == responseData[3])
              {
                ...
                DoorProfile_SetParameter(DOORPROFILE_CHAR2, 8, sys_config.Admin_Pswd);
            ...
            case 0x43:                                                          //  普通按键密码
              if(global.appUpdata_CRC == responseData[3])
              {
                ...
                for(uint8 i=0; i<10; i++)
                {
                  ...
                  DoorProfile_SetParameter(DOORPROFILE_CHAR2, 9, &sys_24c64.General_Pswd[i][0]);
                }
            ...
            case 0x45:                                                          // 开门记录
              if(global.appUpdata_CRC == responseData[3])
              {
                ...
                for(uint8 i=0; i<20; i++)
                {
                  ...
                  DoorProfile_SetParameter(DOORPROFILE_CHAR2, 9, &sys_recode.Open_Recode[i][0]);
                }
            ...
            case 0x46:                                                          
              if(global.appUpdata_CRC == responseData[3])
              {
                ...
                for(uint8 i=0; i<10; i++)
                {
                  ...
                  DoorProfile_SetParameter(DOORPROFILE_CHAR2, 9, &sys_24c64.Once_Single[i][0]);
                }
...
//           osal_setClock(timeReq);
        ...
        case 0x10: // 手机模式是点亮屏幕开启时
        {
          if((responseData[1]==0x4F)&&(responseData[2] == 0x50))
          {
            if(global.safety_Rssi[0] <= global.safety_Rssi[1])				//     只有安全距离下才可以开门
            {
              osal_set_event(bleAutoLock_TaskID, DOOR_OPENDOOR_EVT);   // 开门
              replyUpdata[0] = 0x1F;
              replyUpdata[1] = 0x4B;
              replyUpdata[2] = 0x4F;
...
//          if(global.appDedata_CRC == responseData[8])
          {
            global.app_DownChange = 0x02;
            osal_memcpy(&sys_24c64.General_Pswd[responseData[1]][0], &responseData[2], 9);
            replyUpdata[0] = 0x08;
            replyUpdata[1] = 0x4B;	   
//            replyUpdata[2] = global.appDedata_CRC;
       ...
       case 0x08:																												//  删除一次性密码
        {
          ...
          if(global.authoryMode == 1 )       //只有在管理员权限模式下才可以操作此项
          {
            ...
            case 0x31:		//单次密码无需记录时间，用完立即清空
            {
              ...
              {
                global.app_DownChange = 0x02;
                osal_memcpyz(&sys_24c64.Once_Single[responseData[2]][0], 0xBB, 9);		//  删除
