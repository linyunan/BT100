...
#define SERVAPP_NUM_ATTR_SUPPORTED        22//25
...
static uint8 simpleProfileChar2Props = GATT_PROP_READ | GATT_PROP_WRITE_NO_RSP | GATT_PROP_NOTIFY;

// Characteristic 2 Value
static uint8 simpleProfileChar2[SIMPLEPROFILE_CHAR2_LEN] = { 0, 0, 0, 0, 0 };
static uint8 simpleProfileChar2Len = 0;
...
bStatus_t SimpleProfile_SetParameter( uint8 param, uint8 len, void *value )
{
  ...
  switch ( param )
  {
    ...
    case SIMPLEPROFILE_CHAR2:
      ...
      //  gattAttribute_t *pAttr = GATTServApp_FindAttr(atProfileAttrTbl, GATT_NUM_ATTRS(atProfileAttrTbl),
        //                                              atProfileNValue);
      //  noti.handle = pAttr->handle;
        noti.handle = simpleProfileAttrTbl[5].handle ;
        noti.len = len;
        osal_memcpy((uint8*)noti.value, (uint8*)simpleProfileChar2, len);
...
bStatus_t SimpleProfile_GetParameter( uint8 param, void *value, uint8 *returnBytes)
{
  ...
  switch ( param )
  {
    ...
    case SIMPLEPROFILE_CHAR2:
      ...
      *returnBytes = SIMPLEPROFILE_CHAR2_LEN;
      break;      

    case SIMPLEPROFILE_CHAR3:
      VOID osal_memcpy( value, simpleProfileChar3, SIMPLEPROFILE_CHAR3_LEN );
      *returnBytes = SIMPLEPROFILE_CHAR3_LEN;
      break;  
  ...
  if ( pAttr->type.len == ATT_BT_UUID_SIZE )
  {
    ...
    switch ( uuid )
    {
      ...
      case SIMPLEPROFILE_CHAR2_UUID:
        *pLen = simpleProfileChar2Len;
        VOID osal_memcpy( pValue, pAttr->pValue, simpleProfileChar2Len );
        break;
...
{ 
  ...
  if ( connHandle != LOOPBACK_CONNHANDLE )
  {
    ...
    { 
        GATTServApp_InitCharCfg( connHandle/*INVALID_CONNHANDLE*/, simpleProfileChar2Config );
