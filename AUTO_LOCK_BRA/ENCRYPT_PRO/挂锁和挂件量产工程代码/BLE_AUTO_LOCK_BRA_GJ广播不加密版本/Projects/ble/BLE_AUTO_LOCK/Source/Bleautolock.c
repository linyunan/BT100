...
//      responseData[5] = res[2];
  ...
  if( global.batteryADC > 892 )
  {
    temp =(uint16) global.batteryADC - 892; 
    global.batteryPer = (uint8)( temp*100/276 ) ;
...
//    static uint8 scanRspData2[23] = {0};
    ...
    if( global.batteryNum > 3 && global.batteryNum < 10)
    {
      ...
      Adc_Stop();
      DoorProfile_SetParameter( DOORPROFILE_CHAR2, 3, global.batteryPower );
      if(temp <= 15)
      {
...
//          osal_set_event(bleAutoLock_TaskID, DOOR_ADMIN_PRO_EVT);
        ...
        {
          osal_start_timerEx( bleAutoLock_TaskID, DOOR_ENCRYPT_EVT, 3000 ); 
        }
        else      // 有权限后关闭验证
        {
          osal_stop_timerEx( bleAutoLock_TaskID, DOOR_ENCRYPT_EVT);                 //
...
//  uint16 temp;
  ...
  for(uint8 i=0;i<3;i++)
  {
   ...
   ((BatValue1 > BatValue2) ? BatValue1 : (BatValue0 > BatValue2 ? BatValue2 : BatValue0)));
    ...
    if(global.batteryADC > 1168 && global.batteryADC < 1186)  // 减小
    {
      res[0] = (uint8)(global.batteryADC - 1168);
      AT24C64_I2C_Write(0x1FFE, 1,(uint8 *) &res[0]);
      AT24C64_I2C_Write(0x1FF0, 1,test);
    }
    else if(global.batteryADC < 1168 && global.batteryADC > 1150) // 增加
    {
      res[1] = (uint8)(1168 - global.batteryADC);
      AT24C64_I2C_Write(0x1FFF, 1,(uint8 *) &res[1]);
      AT24C64_I2C_Write(0x1FF0, 1,test);
    }
    else if( global.batteryADC == 1168)
    {
      AT24C64_I2C_Write(0x1FF0, 1,test);
    }
    else
