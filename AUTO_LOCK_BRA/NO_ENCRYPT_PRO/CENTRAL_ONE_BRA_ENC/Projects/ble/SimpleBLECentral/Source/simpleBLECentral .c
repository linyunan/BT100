...
uint8 PHONE_NUM[8] = {0x01, 0x13, 0x77, 0x99, 0x55, 0x64, 0x10, 0x00};
...
//  ssp_HW_KeyInit( key );
  ...
  {
    if( pMsg->msg.handleValueNoti.handle == 0x0028)     //CHAR2
    {
//        ssp_HW_KeyInit( key );
        // 根据 connHandle 查找对应的设备
//        BLE_DEV *p =  simpleBLECentralGetDev(pMsg->connHandle);
//        osal_memcpy(temp2, pMsg->msg.handleValueNoti.value, 3);
//        LL_EXT_Decrypt(key, pMsg->msg.handleValueNoti.value,temp3);
//        sspAesEncryptHW( key, temp2 );
//        sspAesDecryptHW(key, temp2);
        if(osal_memcmp(ADDMODE, pMsg->msg.handleValueNoti.value, 3 ))
        {
...
//          LL_Encrypt(key,PHONE_NUM,PHONE_NUM);
 //         LL_EXT_Decrypt(key,temp,temp2);
          req.handle = 0x0025;

          req.len = 8;

          osal_memcpy(req.value, PHONE_NUM, 8);
          req.sig = 0;
          req.cmd = 0;
