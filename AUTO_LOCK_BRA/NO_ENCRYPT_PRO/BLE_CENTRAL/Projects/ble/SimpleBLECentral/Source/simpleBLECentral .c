...
void Init_Dev_Addr(void)
{
  ...
  {
    osal_memcpy(simpleBLEDevList[i].addr, temp, 6);
  }
}
// Scanning state
...
//        VOID GAPCentralRole_StartDevice( (gapCentralRoleCB_t *) &simpleBLERoleCB );
        ...
        osal_start_timerEx( simpleBLETaskId, SBP_POWERONOFF_EVT, 10000 );
      }
      else
      {
...
//        sspAesDecryptHW(key, temp2);
        if(osal_memcmp(pMsg->msg.handleValueNoti.value, ADDMODE, 3 ))
        {
...
//          LL_Encrypt(key,sys_config.Phone_Num,temp3);
 //         LL_EXT_Decrypt(key,temp,temp2);
          ...
          osal_memcpy(req.value, sys_config.Phone_Num, 16);
          req.sig = 0;
          req.cmd = 0;
          if( sys_config.SYS_ONOFF == TRUE )
          {
            GATT_WriteCharValue( simpleBLEConnHandle[0], &req, simpleBLETaskId );//simpleBLEConnHandle[0]
          }
//          osal_snv_write(0x80, sizeof(SYS_CONFIG), &sys_config);
        }
...
//           VOID osal_memcpy( request, pEvent->deviceInfo.pEvtData, 14  );
           ...
           {
             if( osal_memcmp(&request2[2], REQSCAN_CMP1, 12)  )
             {
                sys_config.UUID_STATUSE = FALSE;
  //              sys_config.Info_Name_Status = TRUE;
             ...
             if (osal_memcmp(&request2[2], REQSCAN_CMP2, 12))
             {
                sys_config.UUID_STATUSE = FALSE;
  //              sys_config.Info_Name_Status = TRUE;
                simpleBLEAddDeviceInfo( pEvent->deviceInfo.addr, pEvent->deviceInfo.addrType );
...
static void simpleBLEAddDeviceInfo( uint8 *pAddr, uint8 addrType )
{
  ...
  if ( simpleBLEScanRes < DEFAULT_MAX_SCAN_RES )
  {
    ...
    {
      if( osal_memcmp( &sys_config.MAC_RECODE[i], pAddr , B_ADDR_LEN ) )
...
//        LCD_WRITE_STRING( bdAddr2Str( peerAddr ), HAL_LCD_LINE_2 ); 
              ...
              simpleBLEState == BLE_STATE_DISCONNECTING)
    {
      ...
      gStatus = GAPCentralRole_TerminateLink( simpleBLEConnHandle[simpleBLEScanIdx-1] );
