 ...
 * CONSTANTS
 */
#define KEY_PORT  P1_4   
   
typedef struct { 

  bool Info_Name_Status ; 
  bool MAC_SAVE_STATUS;       // 是否连接上标志 
  bool ADD_PHONE_MODE;
  bool UUID_STATUSE;
  uint8 MAC_RECODE[10][6];
  uint8 mac_per_index;  //  当前连接的第几个MAC地址
  uint8 mac_index;    // 第几个设备的mac地址索引
  uint8 Phone_Num[16];
  bool SYS_ONOFF;
...
typedef struct{
  uint8 buz_num;          //  蜂鸣器叫的次数
  uint8 led_num;          //  LED
  uint8 init_num;         //  初始化
}GOBAL_VAR;

extern GOBAL_VAR gobal_var;
...
//自动连接任务事件， 注意这是位控制， 所以不能定义成 0x0005----初学者很容易烦这个错误的
#define SBP_AUTO_CONNECT_EVT                          0x0008

#define SBP_SEND_RSSI_EVT                             0x0010

#define SBP_SYSINIT_EVT			        0x0020	 //	系统初始化

#define SBP_POWERONOFF_EVT			0x0040	 //	待机 和唤醒任务

#define SBP_LEDONOFF_EVT			0x0080	 //	灯效果

#define SBP_BUZZER_EVT                          0x0100   //     蜂鸣器

#define SBP_OUTPOWER_EVT                        0x0200   //     待机
...
extern void Sys_Init_Var(void);

extern void Gobal_Init_Var(void);
